...
const GramPanchayat = () => {
    ...
    const submitFormHandler = async (e) => {
        ...
        try {
            const response = await fetch(
                formData?.id ? `http://localhost:5000/api/panchayat/${formData.id}` : "http://localhost:5000/api/new-panchayat",
                {
    ...
    const deletepanchayat = async (id) => {
        ...
        if (!window.confirm("Are you sure you want to delete this Panchayat?")) 
            return;
        try {
            const response = await fetch(`http://localhost:5000/api/panchayat/${id}`, {
             method: "DELETE",
            });
