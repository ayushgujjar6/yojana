...
const Village = () => {
    ...
    const fetchpanchayat = async (taluka_id) => {
        try {
            const response = await fetch(`http://localhost:5000/api/panchayat/${taluka_id}`);
            const data = await  response.json();
    ...
    const fetchvillage = async (taluka_id, panchayat_id) => {
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/village/${taluka_id}/${panchayat_id}`);
            const data = await  response.json();
            setVillageData(data);
        } catch (error) {
            console.error("Error fetching data:", error);
    ...
    const submitFormHandler = async (e) => {
        ...
        try {
            const response = await fetch(
                formData?.id ? `http://localhost:5000/api/village/${formData.id}` : "http://localhost:5000/api/new-village",
                {
    ...
    const deletevillage = async (id) => {
        ...
        if (!window.confirm("Are you sure you want to delete this village?")) 
            return;
        try {
            const response = await fetch(`http://localhost:5000/api/village/${id}`, {
             method: "DELETE",
            });
